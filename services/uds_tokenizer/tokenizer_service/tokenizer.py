...
class TokenizerService:
    ...
    def _create_tokenizer(self, model_identifier: str) -> AnyTokenizer:
        ...
        if os.path.exists(local_model_path) and all(
    ...
    def _download_from_huggingface(
        ...
        try:
            ...
            if os.path.exists(local_model_path) and not os.listdir(local_model_path):
                ...
                logging.info(f"Removed empty directory {local_model_path}")
            logging.error(
                f"Failed to download tokenizer for {model_identifier} from Hugging Face: {e}"
            )
            raise ModelDownloadError(
    ...
    def tokenize_and_process(
        self, prompt: str, add_special_tokens: bool, model_name: str
    ) -> BatchEncoding:
        """
